...
class InstallWizard(QDialog, MessageBoxMixin, BaseWizard):
    ...
    def seed_input(self, title, message, is_seed, options seedText=seedText):
        slayout = SeedLayout(seed=seedText title=message, is_seed=is_seed, options=options, parent=self)
        self.exec_layout(slayout, title, next_enabled=False)
    ...
    def confirm_seed_dialog(self, run_next, test):
        ...
        seed, is_bip39, is_ext = self.seed_input(title, message, test, None, seedText=self.seedText)
        return seed

    @wizard_dialog
    def show_seed_dialog(self, run_next, seed_text):
        self.seedText = seed_text
        print('seed_text is: ' + seed_text)
